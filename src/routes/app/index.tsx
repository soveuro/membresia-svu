...
export default component$( function () {
  ...
  useVisibleTask$(async()=>{
    const photo = await fetch("https://res.cloudinary.com/daydr8ynq/image/upload/h_200,w_200/v1/doctors/"+27755829)
        ...
        <div class={"hidden overflow-auto sm:min-h-screen sm:relative sm:w-1/4 w-full sm:flex flex-col justify-between items-center text-center text-black  text-lg sm:text-2xl  p-4 h-1/2 bg-[#ffff00]"+` ${isBarVisible.value&&"menu"}`}>
          ...
          <div class="sm:h-1/6 w-full items-center flex flex-col justify-start flex-auto">
            <div class="rounded-xl mb-8 overflow-hidden">
              <img src={userPhoto.value} class="h-[200px] w-[200px]" alt="" />
            </div>
            <p class='mb-8'>{user?.sex==="m"? "Dr.":"Dra."} {user?.first_names} {user?.last_names}</p>
            <buttton onClick$={()=>nav("/app/modificar")} class="px-8 cursor-pointer font-bold rounded w-full mb-8 bg-black text-[#ffff00] text-base">Editar datos</buttton>
            ...
            <buttton onClick$={()=>nav("/app/pagar")} class="px-8 cursor-pointer font-bold rounded w-full mb-8 bg-black text-[#ffff00] text-base">Registrar pago</buttton>
            <buttton onClick$={()=>nav("/ingresar")} class="px-8 cursor-pointer font-bold rounded w-full mb-8 bg-black text-[#ffff00] text-base">Cerrar sesión</buttton>
          </div>
          <Link href="/">
        ...
        <div class="sm:w-3/4 sm:px-8 px-8 w-full py-8 overflow-auto sm:overflow-hidden min-h-screen sm:h-full bg-black flex flex-col sm:flex-row justify-start items-center">
          <div class='sm:w-[calc(50%-16px*2)] mb-8 w-full h-full sm:m-4 bg-[#ffff00]' style={{backdropFilter:"blur(10px)",backgroundColor:"#ffff00"}}>
            ...
            <ul class="flex flex-col justify-evenly m-4">
              ...
              <li class='sm:mt-[3vh] font-semibold mt-4'>Sexo: {user?.sex?.toLowerCase() ==="ne" ? "no especifica" : user.value?.sex?.toLowerCase() ==="m"? "Masculino":"Femenino"}</li>
              ...
              <li class='sm:mt-[3vh] font-semibold mt-4'>Cedula: {user?.id}</li>
              <li class='sm:mt-[3vh] font-semibold mt-4'>Fecha de ingreso: {user?.date_of_enrollment}</li>
              <li class='sm:mt-[3vh] font-semibold mt-4'>Relación: {user?.relation}</li>
            </ul>
          </div>
