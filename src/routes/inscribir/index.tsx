...
const register = server$(async function (userData:any){
  this.cacheControl({
    public: false,
    maxAge: 0,
    sMaxAge: 0,
  ...
  userData.date_of_enrollment = new Date().toISOString().slice(0, 19).replace('T', ' ').split(" ")[0];
      ...
      <div class="flex sm:overflow-hidden relative overflow-auto sm:flex-row flex-col h-screen w-full">
        ...
        <form preventdefault:submit onSubmit$={onRegister} class="min-w-full mt-8 mb-4 sm:overflow-auto sm:overflow-x-hidden text-white">
          ...
          <div class='flex text-black mb-5 flex-col flex-auto relative'>
              <label for="relation" class="text-white mb-3">Tipo de registro</label>
              <select id="relation" required class="bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700  dark:border-gray-600  dark:placeholder-gray-400  dark:  dark:focus:ring-blue-500 dark:focus:border-blue-500">
          ...
          <div class="grid md:grid-cols-2 md:gap-6">
            <div class="relative z-0 w-full mb-5 group">
                ...
                <label for="egress" class="peer-focus:font-medium absolute text-sm  dark duration-300 transform -translate-y-6 translate-x-[-10rem] scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus :dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 peer-focus:translate-x-0 w-full">Universidad de egreso como cirujano</label>
            </div>
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" name="egress_speciality" id="egress_speciality" class="block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:  dark:border-gray-600  dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                <label for="egress_speciality" class="peer-focus:font-medium absolute text-sm  dark duration-300 transform -translate-y-6 scale-75 translate-x-[-10rem]  top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus :dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 w-full peer-focus:translate-x-0 peer-focus:-translate-y-6">Hospital de egreso como especialista</label>
            </div>
          </div>
          <div class='flex text-black mb-5 flex-col flex-auto relative'>
          ...
          <Link href="/ingresar" class=" inline-block"><p class="text-yellow-200">¿Ya estas inscrito?</p></Link>
          </div>
